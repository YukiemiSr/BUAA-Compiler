    ...
    if (!regList.empty()) {
        for (auto item: regList) {
            if (item->depth == 0) {
                ...
                s1 += "[" + to_string(item->dim1Value) + " x i32] *";
            }
...
void printCalc(LexerType type, Register *left, Register *right, Register *ans) {
    if(type == NOT) {
        ...
        str = str + switchLexer(type) + " i1 ";
        str = str + left->printRegister() + ", true";
        if (curTable->id != 1) {
            output.push_back(str);
            //fll << str << endl;
        }
...
void printAllocaDims(int regCnt, Symbol *sym) {
    if (sym->depth == 1) {
        string l = "  %" + to_string(regCnt) + " = alloca ";
        l += "i32*";
        output.push_back(l);
    } else if (sym->depth == 2) {
        string l = "  %" + to_string(regCnt) + " = alloca [";
        l += to_string(sym->firstDimLength) + " x i32]*";
        output.push_back(l);
    }
}
...
void printStoreDims(int regCnt, Symbol *sym) {
    if (sym->depth == 1) {
        string l = "  store ";
        l += "i32* %" + to_string(regCnt);
        l += ", i32* * %" + to_string(sym->regId);
        output.push_back(l);
    } else if (sym->depth == 2) {
        string l = "  store ";
        l += "[ " + to_string(sym->firstDimLength) + " x i32]* %" + to_string(regCnt);
        l += ", [" + to_string(sym->firstDimLength) + " x i32]* * %" + to_string(sym->regId);
        output.push_back(l);
    }
}
...
void printLoadAllocDims(int regCnt, Symbol *sym) {
    if (sym->depth == 1) {
        string l = "  %" + to_string(regCnt) + " = load ";
        l += "i32* ";
        l += ", i32* * %" + to_string(sym->regId);
        output.push_back(l);
    } else if (sym->depth == 2) {
...
void printCallFunc(Register *type, vector<Register *> regList) {
    auto list = tableMap[1]->funcMap[type->name];
    string s;
    if (type->type == 1) s = s + "  " + type->printRegister() + " = call i32 @" + type->name + "(";
    else s = s + "  " + "call void @" + type->name + "(";
    int cnt = 0;
    if (!regList.empty()) {
        for (auto item: regList) {
            if (list[cnt]->depth == 0) {
                s = s + "i32 " + item->printRegister();
            } else if (list[cnt]->depth == 1) {
                s = s + "i32* " + item->printRegister();
            } else if (list[cnt]->depth == 2) {
                s = s + "[ " + to_string(list[cnt]->firstDimLength) + " x i32]* " + item->printRegister();
            }
